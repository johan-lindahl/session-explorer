...
- **Queues pane** (`Static`, `id="queues"`, under the tree) is **read-only** and
  ...
  **nothing** (zero-footprint). An explicit `q` with nothing to show surfaces a
  ...
  for every ticket. Names are **truncated** to `_QUEUE_NAME_MAX` (20) with an
...
All milestones below are **shipped** (current release: v1.17.1). The table is kept as a delivery record of what each one added.
