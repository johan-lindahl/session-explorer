...
| `f` | Full-text search the **current project's** transcript bodies (spec below). Opens a modal listing matching sessions with in-context snippets; `ctrl+u` includes unnamed; Enter moves the tree cursor to the pick (no resume). Distinct from `/`, which only matches cached metadata. |
...
### Full-text conversation search (`f`, v1.19.0)

`/` matches only cached metadata; `f` searches the *conversation bodies* that Claude actually wrote to disk. It answers "which of my sessions discussed X, and what was said?" without opening anything.

- **Scope: the current project only.** `action_search` resolves the project from the tree cursor (`_project_and_prefix_for_cursor`) and gathers every session grouped under that repo root via `tree_model.session_root` (so worktrees collapse in). Cross-project search is deliberately out of scope for v1.
- **What's matched.** `search.iter_text_messages` yields `(role, text)` for `user`/`assistant` messages only, from string `content` or the `type=="text"` parts of a list — the same extraction shape as `summary.build_digest`, minus truncation. Tool calls, tool results, thinking, snapshots, and system lines are skipped. Match is case-insensitive substring (no regex/phrase/ranking in v1).
- **Live scan, no index.** `search.py` is Textual-free (unit-tested like `root_guard.py`/`gc.py`) and reads the JSONL at search time. There is **no fourth sidecar** — nothing to keep fresh or that can go stale. A single project's scan is sub-second; a full 370-session / ~885 MB cross-project scan measured ~2.4 s, so per-project is comfortably interactive.
- **Results.** `search.search_project` returns sessions with ≥1 hit, newest first, each carrying a capped snippet list (`MAX_SNIPPETS_PER_SESSION`, "+N more" overflow) built from `SNIPPET_WIDTH`-char whitespace-collapsed windows with the match highlighted. `SearchScreen` renders them as an `OptionList` (one option per session); the input's Enter runs the scan in a **guarded worker** (a scan failure logs-and-skips, never exits the app), `ctrl+u` toggles including unnamed sessions (default off, matching the tree), and selecting an option dismisses with the sid.
- **Non-destructive selection.** On dismiss, `action_search`'s callback sets `_pending_select_sid` and repopulates so the tree cursor lands on the pick (revealing unnamed rows by switching to view-mode 2 if needed). It does **not** resume — resume stays a separate, deliberate Enter on the tree row.
- **Keybinding discipline.** `f` is kept in sync across `BINDINGS`, `check_action` (blocked while any modal is up, and while the `/` filter input is focused so a typed `f` stays in the text), and `_help_text`.
...
All milestones below are **shipped** (current release: v1.19.3). The table is kept as a delivery record of what each one added.
