...
def collect_garbage(index_path: str, *, retention_days: int = 30,
                    dry_run: bool = False,
                    now: "datetime | None" = None) -> dict:
    """Delete unnamed sessions older than `retention_days`.

    Returns {"removed": [session_id, ...], "skipped_live": int, "dry_run": bool}.
    ...
    if dry_run:
        ...
        for sid, entry in data.get("sessions", {}).items():
            ...
            if age is not None and age.timestamp() >= cutoff:
                continue
            removed.append(sid)
        return {"removed": removed, "skipped_live": skipped_live, "dry_run": True}
