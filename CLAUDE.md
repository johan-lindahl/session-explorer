...
- **A new session's launch failure is captured, not silent.** New-session windows run `exec claude … 2>` a per-sid errfile; ~1.5s after launch `_check_launch` (`tui.py`) verifies the session is alive and, on death (typically `claude -w` failing to create its worktree), surfaces+logs the captured stderr and stamps the index row's `last_launch_error` (shown in the preview). A transcript-less stub is started fresh on Enter via `--session-id` (`_start_stub_fresh`), never `--resume`. `record_session` clears `last_launch_error` once a transcript appears.
